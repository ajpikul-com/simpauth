import { assert } from 'chai';
import * as galleta from "../src/index";


const sampleCookie: string = ""

describe("GetCookie Tests", () => {
  it("should return a json object with a string embedded 'TestValue'", () => {
    const o: galleta.cookie | null = galleta.decodeCookie(sampleCookie);
    if (o == null) {
      assert.fail("null", "galleta.cookie", "We didn't get a cookie object back")
    }
    const v: string = o!.StateString;
    if ( v !== "TestValue") {
      assert.fail(v, "TestValue", "String not correct");
    }
  });
});
